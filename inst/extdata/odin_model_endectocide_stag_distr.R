...
initial(Ev_1_F1[]) = init_Ev/spor_len * mv0
initial(Ev_1_F2[]) = 0 #init_Ev/spor_len * mv0/2
...
initial(Iv_1_F1) = init_Iv *mv0
initial(Iv_1_F2) = 0 #init_Iv *mv0/2
...
initial(Ev_2_F1[]) = init_Ev/spor_len * mv0
initial(Ev_2_F2[]) = 0 #init_Ev/spor_len * mv0/2
...
initial(Iv_2_F1) = init_Iv *mv0
initial(Iv_2_F2) = 0 #init_Iv *mv0/2
...
Ivtot = Iv_1_F1 + Iv_1_F2 + sum(Ix_1_F1) + sum(Ix_1_F2) + Iv_2_F1 + Iv_2_F2 + sum(Ix_2_F1) + sum(Ix_2_F2)
Evtot = sum(Ev_1_F1) + sum(Ev_1_F2) + sum(Ex_1_F1) + sum(Ex_1_F2) + sum(Ev_2_F1) + sum(Ev_2_F2) + sum(Ex_2_F1) + sum(Ex_2_F2)
Svtot = Sv_1 + Sv_1_F1 + Sv_1_F2 + sum(Sx_1_F1) + sum(Sx_1_F2) + Sv_2 + Sv_2_F1 + Sv_2_F2 + sum(Sx_2_F1) + sum(Sx_2_F2)
...
#Sx_1_dead = sum(Sx_1_F1_dead) + sum(Sx_1_F2_dead)
#Ex_1_dead = sum(Ex_1_F1_dead) + sum(Ex_1_F2_dead)
#Ix_1_dead = sum(Ix_1_F1_dead) + sum(Ix_1_F2_dead)
#mvx_1_dead = Sx_1_dead + Ex_1_dead + Ix_1_dead
#
#Sx_2_dead = sum(Sx_2_F1_dead) + sum(Sx_2_F2_dead)
#Ex_2_dead = sum(Ex_2_F1_dead) + sum(Ex_2_F2_dead)
#Ix_2_dead = sum(Ix_2_F1_dead) + sum(Ix_2_F2_dead)
#mvx_2_dead = Sx_2_dead + Ex_2_dead + Ix_2_dead
...
deriv(Sv_1) =               betaa - mu*Sv_1 - avhc*Sv_1

deriv(Sv_1_F1) =            if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len))  (avhc-FOIv)*(1-ivm_cov)*Sv_1 - avhc*Sv_1_F1 - mu*Sv_1_F1    else (avhc - FOIv)*Sv_1 - avhc*Sv_F1 - mu*Sv_F1
...
deriv(Ex_1_F1[2:spor_len, 1:eff_len]) =   if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1-1 + eff_len)) 1/tau_v*Ex_1_F1[i-1,j] - 1/tau_v*Ex_1_F1[i,j] - mu_vi[j]*Ex_1_F1[i,j] - avhc*Ex_1_F1[i,j] else  1/tau_v*Ex_1_F1[i-1,j]  - (1/tau_v + avhc)*Ex_1_F1[i,j] - mu_vi[j]*Ex_1_F1[i,j]
...
deriv(Sv_2) =               betaa - mu*Sv_2 - avhc*Sv_2

deriv(Sv_2_F1) =            if (t >= (IVRM_sr_2)   && t < (IVRM_sr_2 + eff_len))  (avhc-FOIv)*(1-ivm_cov)*Sv_2 - avhc*Sv_2_F1 - mu*Sv_2_F1    else (avhc - FOIv)*Sv_2 - avhc*Sv_F1 - mu*Sv_F1
...
deriv(Ex_2_F1[2:spor_len, 1:eff_len]) =   if (t >= (IVRM_sr_2 + j -1)  &&  t < (IVRM_sr_2 + j ) && t < (IVRM_sr_2-1 + eff_len)) 1/tau_v*Ex_2_F1[i-1,j] - 1/tau_v*Ex_2_F1[i,j] - mu_vi[j]*Ex_2_F1[i,j] - avhc*Ex_2_F1[i,j] else  1/tau_v*Ex_2_F1[i-1,j]  - (1/tau_v + avhc)*Ex_2_F1[i,j] - mu_vi[j]*Ex_2_F1[i,j]


deriv(Ex_2_F2[1, 1:eff_len]) =            if (t >= (IVRM_sr_2 + j -1)  &&  t < (IVRM_sr_2 + j ) && t < (IVRM_sr_2 + eff_len)) ivm_cov*FOIv*(Sv_2_F1 + Sv_2_F2) + FOIv*(Sx_2_F1[j] + Sx_2_F2[j]) + avhc*ivm_cov*(Ev_2_F1[i] + Ev_2_F2[i]) + avhc*Ex_2_F2[i,j] - 1/tau_v*Ex_2_F2[i,j] - mu_vi[j]*Ex_2_F2[i,j] else  avhc*Ex_2_F2[i,j] + FOIv*(Sx_2_F1[j] + Sx_2_F2[j]) - 1/tau_v*Ex_2_F2[i,j] - mu_vi[j]*Ex_2_F2[i,j]

deriv(Ex_2_F2[2:spor_len, 1:eff_len]) =   if (t >= (IVRM_sr_2 + j -1)  &&  t < (IVRM_sr_2 + j ) && t < (IVRM_sr_2 + eff_len)) 1/tau_v*Ex_2_F2[i-1,j] + avhc*ivm_cov*(Ev_2_F1[i] + Ev_2_F2[i]) + avhc*Ex_2_F2[i,j] -	1/tau_v*Ex_F2[i,j] - mu_vi[j]*Ex_2_F2[i,j]  else avhc*Ex_2_F1[i,j] + 1/tau_v*Ex_2_F2[i-1,j] - mu_vi[j]*Ex_2_F2[i,j]  -1/tau_v*Ex_2_F2[i,j]
...
deriv(Ix_2_F2[1:eff_len]) = if (t >= (IVRM_sr_2 + i -1)  &&  t < (IVRM_sr_2 + i) && t < (IVRM_sr_2 + eff_len))  avhc*ivm_cov*(Iv_2_F1 + Iv_2_F2)  + 1/tau_v*Ex_2_F2[spor_len,i]  + avhc*Ix_2_F1[i] - mu_vi[i]*Ix_2_F2[i]  else 1/tau_v*Ex_2_F2[spor_len, i] + avhc*Ix_F1[i] - mu_vi[i]*Ix_2_F2[i]
