#modifications for a staggered delivery
...
EIR_1[,,] <- av_human_1[k] * rel_foi[j] * foi_age[i] * Ivtot_1/omega
EIR_2[,,] <- av_human_2[k] * rel_foi[j] * foi_age[i] * Ivtot_2/omega

EIR[,,] <- EIR_1[i,j,k] + EIR_2[i,j,k] #goes into human equations
EIR_tot <- sum(EIR_1[,,], EIR_2[,,])
...
Q0_species1 <- user() # proportion of anthropophagy. Probability of biting a person.
Q0_species2 <- user()
Q0_species <- user()
bites_Bed <- user()
...
#let resistance be the same in the 3 populations
r_ITN0 <- user()
d_ITN0 <- user()
d_IRS0 <- user()
r_IRS0 <- user()
...
zbar_species1 <- Q0_species1*zh
zbar_species2 <- Q0_species2*zh
# W (wbar) - average probability of mosquito successfully feeding during single attempt
wbar_species1 <- 1 - Q0_species1 + Q0_species1*wh
wbar_species2 <- 1 - Q0_species2 + Q0_species2*wh
...
Q_2 <- 1-(1-Q0_2)/wbar
Q_3 <- 1-(1-Q0_3)/wbar
av_1 <- fv*Q_1 # biting rate on humans
av_2 <- fv*Q_2
av_3 <- fv*Q_3
...
#av_mosq for each species
dim(av_mosq_1) <- num_int
av_mosq_1[1:num_int] <- av_1*w[i]/wh
av_mosq_2[1:num_int] <- av_2*w[i]/wh
av_mosq_3[1:num_int] <- av_3*w[i]/wh
...
dim(av_human_1) <- num_int
(av_human_1)[1:num_int] <- av_1*yy[i]/wh
dim(av_human_2) <- num_int
(av_human_2)[1:num_int] <- av_2*yy[i]/wh
dim(av_human_3) <- num_int
(av_human_3)[1:num_int] <- av_3*yy[i]/wh
