...
#IVRM_sr_1 = interpolate(ttt, IVRM_start, "constant")

IVRM_start_1[]<-user()
dim(IVRM_start_1) <- length(ttt)

IVRM_sr_1_1 = interpolate(ttt, IVRM_start_1, "constant")

IVRM_start_2[]<-user()
dim(IVRM_start_2) <- length(ttt)

IVRM_sr_1_2 = interpolate(ttt, IVRM_start_2, "constant")

IVRM_sr_1_3 = interpolate(ttt, IVRM_start_3)

deriv(Sv) =               betaa - mu*Sv - avhc*Sv

deriv(Sv_F1) =            if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len))  (avhc-FOIv)*(1-ivm_cov)*Sv/3 - avhc*Sv_F1/3 - mu*Sv_F1/3    else if (t >= (IVRM_sr_2)   && t < (IVRM_sr_2 + eff_len))  (avhc-FOIv)*(1-ivm_cov)*Sv/3 - avhc*Sv_F1/3 - mu*Sv_F1/3 else if (t >= (IVRM_sr_3)   && t < (IVRM_sr_3 + eff_len))  (avhc-FOIv)*(1-ivm_cov)*Sv/3 - avhc*Sv_F1/3 - mu*Sv_F1/3 else (avhc - FOIv)*Sv - avhc*Sv_F1 - mu*Sv_F1

deriv(Sv_F2) =           if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) (avhc - FOIv)*(1-ivm_cov)*Sv_F1/3 -(FOIv*(1-ivm_cov) + avhc*ivm_cov)*Sv_F2/3 - mu*Sv_F2/3 else if (t >= (IVRM_sr_2)   && t < (IVRM_sr_2 + eff_len)) (avhc - FOIv)*(1-ivm_cov)*Sv_F1/3 -(FOIv*(1-ivm_cov) + avhc*ivm_cov)*Sv_F2/3 - mu*Sv_F2/3 else if (t >= (IVRM_sr_3)   && t < (IVRM_sr_3 + eff_len)) (avhc - FOIv)*(1-ivm_cov)*Sv_F1/3 -(FOIv*(1-ivm_cov) + avhc*ivm_cov)*Sv_F2/3 - mu*Sv_F2/3 else (avhc-FOIv)*Sv_F1 - FOIv*Sv_F2 - mu*Sv_F2

deriv(Sx_F1[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len )) ivm_cov*(avhc-FOIv)*Sv/3 - avhc*Sx_F1[i]/3 - mu_vi[i]*Sx_F1[i]/3 else if (t >= (IVRM_sr_2 + i -1)  &&  t < (IVRM_sr_2 + i) && t < (IVRM_sr_2 + eff_len )) ivm_cov*(avhc-FOIv)*Sv/3 - avhc*Sx_F1[i]/3 - mu_vi[i]*Sx_F1[i]/3 else if (t >= (IVRM_sr_3 + i -1)  &&  t < (IVRM_sr_3 + i) && t < (IVRM_sr_3 + eff_len )) ivm_cov*(avhc-FOIv)*Sv/3 - avhc*Sx_F1[i]/3 - mu_vi[i]*Sx_F1[i]/3    else - mu_vi[i]*Sx_F1[i] - avhc*Sx_F1[i]

deriv(Sx_F2[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len )) ivm_cov*(avhc-FOIv)*(Sv_F1 + Sv_F2) + (avhc - FOIv)*Sx_F1[i] - FOIv*Sx_F2[i] - mu_vi[i]*Sx_F2[i]     else  (avhc- FOIv)*Sx_F1[i] - mu_vi[i]*Sx_F2[i] - FOIv*Sx_F2[i]


deriv(Ev_F1[1])  =           if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) FOIv*(1-ivm_cov)*Sv  - mu*Ev_F1[i] - avhc*Ev_F1[i] - 1/tau_v*Ev_F1[i]  else FOIv*Sv - avhc*Ev_F1[i] - 1/tau_v*Ev_F1[i] - mu*Ev_F1[i]

deriv(Ev_F1[2:spor_len]) =   if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len))  1/tau_v*Ev_F1[i-1] - avhc*Ev_F1[i] - 1/tau_v*Ev_F1[i] - mu*Ev_F1[i]  else 1/tau_v*Ev_F1[i-1] - 1/tau_v*Ev_F1[i] - avhc*Ev_F1[i]- mu*Ev_F1[i]


deriv(Ev_F2[1]) =            if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) avhc*(1-ivm_cov)*Ev_F1[i] + (FOIv*(1-ivm_cov))*(Sv_F1+Sv_F2)  - mu*Ev_F2[i] - avhc*ivm_cov*Ev_F2[i] - 1/tau_v*Ev_F2[i]  else avhc*Ev_F1[i] + FOIv*(Sv_F1 + Sv_F2) - 1/tau_v*Ev_F2[i] - mu*Ev_F2[i]

deriv(Ev_F2[2:spor_len]) =   if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len))  1/tau_v*Ev_F2[i-1] + avhc*(1-ivm_cov)*Ev_F1[i] - avhc*ivm_cov*Ev_F2[i] - 1/tau_v*Ev_F2[i] - mu*Ev_F2[i]   else 1/tau_v*Ev_F2[i-1] + avhc*Ev_F1[i] - 1/tau_v*Ev_F2[i] - mu*Ev_F2[i]


deriv(Ex_F1[1, 1:eff_len]) =            if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) FOIv*ivm_cov*Sv  - 1/tau_v*Ex_F1[i,j] - mu_vi[j]*Ex_F1[i,j] - avhc*Ex_F1[i,j] else - 1/tau_v*Ex_F1[i,j] - mu_vi[j]*Ex_F1[i,j]  - avhc*Ex_F1[i,j]

deriv(Ex_F1[2:spor_len, 1:eff_len]) =   if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) 1/tau_v*Ex_F1[i-1,j] - 1/tau_v*Ex_F1[i,j] - mu_vi[j]*Ex_F1[i,j] - avhc*Ex_F1[i,j] else  1/tau_v*Ex_F1[i-1,j]  - (1/tau_v + avhc)*Ex_F1[i,j] - mu_vi[j]*Ex_F1[i,j]


deriv(Ex_F2[1, 1:eff_len]) =            if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) ivm_cov*FOIv*(Sv_F1 + Sv_F2) + FOIv*(Sx_F1[j] + Sx_F2[j]) + avhc*ivm_cov*(Ev_F1[i] + Ev_F2[i]) + avhc*Ex_F1[i,j] - 1/tau_v*Ex_F2[i,j] - mu_vi[j]*Ex_F2[i,j] else  avhc*Ex_F1[i,j] + FOIv*(Sx_F1[j] + Sx_F2[j]) - 1/tau_v*Ex_F2[i,j] - mu_vi[j]*Ex_F2[i,j]

deriv(Ex_F2[2:spor_len, 1:eff_len]) =   if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) 1/tau_v*Ex_F2[i-1,j] + avhc*ivm_cov*(Ev_F1[i] + Ev_F2[i]) + avhc*Ex_F1[i,j] -	1/tau_v*Ex_F2[i,j] - mu_vi[j]*Ex_F2[i,j]  else avhc*Ex_F1[i,j] + 1/tau_v*Ex_F2[i-1,j] - mu_vi[j]*Ex_F2[i,j]  -1/tau_v*Ex_F2[i,j]


deriv(Iv_F1) =  if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) 1/tau_v*Ev_F1[spor_len] - (mu + avhc)*Iv_F1     else  1/tau_v*Ev_F1[spor_len] -   (mu + avhc)*Iv_F1

deriv(Iv_F2) =  if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) 1/tau_v*Ev_F2[spor_len] + avhc*(1-ivm_cov)*Iv_F1 - (mu + ivm_cov*avhc)*Iv_F2     else  1/tau_v*Ev_F2[spor_len] + avhc*Iv_F1 -  mu*Iv_F2

deriv(Ix_F1[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len)) 1/tau_v*Ex_F1[spor_len,i] - (avhc + mu_vi[i])*Ix_F1[i]   else 1/tau_v*Ex_F1[spor_len,i] - (mu_vi[i] + avhc)*Ix_F1[i]

deriv(Ix_F2[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len))  avhc*ivm_cov*(Iv_F1 + Iv_F2)  + 1/tau_v*Ex_F2[spor_len,i]  + avhc*Ix_F1[i] - mu_vi[i]*Ix_F2[i]  else 1/tau_v*Ex_F2[spor_len, i] + avhc*Ix_F1[i] - mu_vi[i]*Ix_F2[i]

#track IVM-killed  mosquitoes#####
deriv(Sx_F1_dead[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len )) mu_vi[i]*Sx_F1[i] else mu_vi[i]*Sx_F1[i]
deriv(Sx_F2_dead[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len )) mu_vi[i]*Sx_F2[i] else mu_vi[i]*Sx_F2[i]

deriv(Ex_F1_dead[1,1:eff_len]) =           if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) mu_vi[j]*Ex_F1[i,j] else mu_vi[j]*Ex_F1[i,j]
deriv(Ex_F1_dead[2:spor_len, 1:eff_len]) = if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) mu_vi[j]*Ex_F1[i,j] else mu_vi[j]*Ex_F1[i,j]

deriv(Ex_F2_dead[1, 1:eff_len]) =            if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) mu_vi[j]*Ex_F2[i,j] else mu_vi[j]*Ex_F2[i,j]
deriv(Ex_F2_dead[2:spor_len, 1:eff_len]) =   if (t >= (IVRM_sr_1 + j -1)  &&  t < (IVRM_sr_1 + j ) && t < (IVRM_sr_1 + eff_len)) mu_vi[j]*Ex_F2[i,j] else mu_vi[j]*Ex_F2[i,j]

deriv(Ix_F1_dead[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len)) mu_vi[i]*Ix_F1[i] else mu_vi[i]*Ix_F1[i]
deriv(Ix_F2_dead[1:eff_len]) = if (t >= (IVRM_sr_1 + i -1)  &&  t < (IVRM_sr_1 + i) && t < (IVRM_sr_1 + eff_len)) mu_vi[i]*Ix_F2[i] else mu_vi[i]*Ix_F2[i]


#track non-IVM-killed mosquitoes####
deriv(Sv_dead) = mu*Sv
deriv(Sv_F1_dead) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Sv_F1 else mu*Sv_F1
deriv(Sv_F2_dead) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Sv_F2 else mu*Sv_F2

deriv(Ev_F1_dead[1]) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Ev_F1[i] else mu*Ev_F1[i]
deriv(Ev_F1_dead[2:spor_len]) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Ev_F1[i] else  mu*Ev_F1[i]

deriv(Ev_F2_dead[1]) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Ev_F2[i] else mu*Ev_F2[i]
deriv(Ev_F2_dead[2:spor_len]) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Ev_F2[i] else mu*Ev_F2[i]

deriv(Iv_F1_dead) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Iv_F1 else mu*Iv_F1
deriv(Iv_F2_dead) = if (t >= (IVRM_sr_1)   && t < (IVRM_sr_1 + eff_len)) mu*Iv_F2 else mu*Iv_F2
