...
ivm_cov_par <- user() #initial coverage from first MDA
ivm_cov_groups <- user()
ivm_min_age <- user()
ivm_max_age <- user()
ivm_cov_par1 <- if (t >= (IVRM_sr + i -1)  &&  t < (IVRM_sr + i) && t < (IVRM_sr + eff_len )) ivm_cov_par else ivm_cov_par*ivm_cov_groups
ivm_cov = ivm_cov_par1*(exp(-ivm_min_age/21) - exp(-ivm_max_age/21)) #not sure if you would do this additional step if going to modify the hazard ratios too.
