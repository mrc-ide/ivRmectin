...
#Evxtot_dead_natural = Evx_dead_natural
...
#endec_killing <-  (endec_mu*exp(-wane*(t - IVRM_sr)))
...
deriv(Svx_dead) <- if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len))  endec_mu*Sv  else 0
deriv(Evx_dead[1]) <- if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len)) endec_mu*Ev[1] else 0
deriv(Evx_dead[2:10]) <-if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len)) endec_mu *Ev[i] else 0
deriv(Ivx_dead) <- if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len)) endec_mu*Iv else 0
