...
initial(Svx_dead) <- 0
initial(Evx_dead[1:10]) <- 0
dim(Evx_dead) <- 10
initial(Ivx_dead) <- 0
initial(D_kill) <- 0

initial(Svx_dead_natural) <- 0
initial(Evx_dead_natural[1:10]) <- 0
dim(Evx_dead_natural) <- 10
initial(Ivx_dead_natural) <- 0
...
Ivxtot_dead = Ivx_dead
Evxtot_dead = sum(Evx_dead)
Svxtot_dead = Svx_dead

mvx_dead = Ivxtot_dead + Evxtot_dead + Svxtot_dead

Ivxtot_dead_natural = Ivx_dead_natural
Evxtot_dead_natural = sum(Evx_dead_natural)
Svxtot_dead_natural = Svx_dead_natural

mvx_dead_natural = Ivxtot_dead_natural + Evxtot_dead_natural + Svxtot_dead_natural
...
#endec_on_dur[] <- user()
#dim(endec_on_dur) <- user()
IVRM_sr = interpolate(ttt, IVRM_start, "constant")
#t_imp = interpolate(ttt, endec_on_dur, "constant")
#endec_killing <- mu + (endec_mu*avhc*ivm_cov)
#t_imp <- t - IVRM_sr
#t_imp <- user()
#dim(t_imp) <- length(ttt)
#time_for_endec <- endec_on_dur
#endec_on_dur <- user()
#t_imp[] <- user()
#dim(t_imp) <- user()
#t_imp_on = interpolate(ttt, t_imp, "linear")
wane <- user()
#dim(t_imp) <-
#endec_killing <- (mu + endec_mu)*exp(-wane)

#endec_killing <- mu + (endec_mu*exp(-wane*(t - endec_on)))
#endec_killing <-  (endec_mu*exp(-wane*(t - IVRM_sr))) #for malariasim
endec_killing <- endec_mu
#endec_on <- user()
#dim(endec_killing) <- length(ttt)
endec_mu <- user()
#endec_killing <- user()


deriv(Sv) <-  if (t >= (IVRM_sr)   && t <= (IVRM_sr + eff_len)) -(FOIv*Sv) - endec_killing*Sv - mu*Sv  + betaa else - (FOIv*Sv) - mu*Sv + betaa
deriv(Ev[1]) <- if (t >= (IVRM_sr)   && t <= (IVRM_sr + eff_len)) (FOIv*Sv) - Ev[1] - endec_killing*Ev[1] - mu*Ev[1] else (FOIv*Sv) - Ev[1] - mu*Ev[1]
deriv(Ev[2:10]) <-if (t >= (IVRM_sr)   && t <= (IVRM_sr + eff_len)) Ev[i-1] - Ev[i] - endec_killing *Ev[i] - mu*Ev[i] else Ev[i-1] - Ev[i] - mu*Ev[i]
deriv(Iv) <- if (t >= (IVRM_sr)   && t <= (IVRM_sr + eff_len)) Ev[10] - endec_killing*Iv  - mu*Iv else Ev[10] - mu*Iv

#track the endec-killed mosq
deriv(Svx_dead) <- if (t >= (IVRM_sr)   && t<= (IVRM_sr + eff_len))  endec_killing*Sv  else 0
deriv(Evx_dead[1]) <- if (t >= (IVRM_sr)   && t <= (IVRM_sr + eff_len)) endec_killing*Ev[1] else 0
deriv(Evx_dead[2:10]) <-if (t >= (IVRM_sr)   && t <= (IVRM_sr + eff_len)) endec_killing *Ev[i] else 0
deriv(Ivx_dead) <- if (t >= (IVRM_sr)   && t <= (IVRM_sr + eff_len)) endec_killing*Iv else 0


deriv(D_kill) <- if (t >= (IVRM_sr) && t <= (IVRM_sr+eff_len)) endec_killing*(Sv + sum(Ev[1:10]) + Iv) else 0


#track natural deaths (in endec-killing period)
deriv(Svx_dead_natural) <- if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len))  mu*Sv  else 0
deriv(Evx_dead_natural[1]) <- if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len)) mu*Ev[1] else 0
deriv(Evx_dead_natural[2:10]) <-if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len)) mu *Ev[i] else 0
deriv(Ivx_dead_natural) <- if (t >= (IVRM_sr)   && t < (IVRM_sr + eff_len)) mu*Iv else 0
...
output(mv0) <- mv0
output(endec_mu) <- endec_mu
output(endec_killing) <- endec_killing
output(mvx_dead) <- mvx_dead
output(mvx_dead_natural) <- mvx_dead_natural
