#in this model, we don't need hazards, just apply a transformation to mu
## MODEL VARIABLES
##------------------------------------------------------------------------------
...
output(mvx_dead) <- IVM_dead
output(mv_dead) <- mv_dead
output(mv_dead_all_int) <- mv_dead_all_int
