...
ivm_min_age <- user()
ivm_max_age <- user()
prop_human_HR_threshold[] <- user() #prop of people with HR > 1 at any timestep
dim(prop_human_HR_threshold) <- length(ttt)

prop_human_HR = interpolate(ttt, prop_human_HR_threshold, "constant")
ivm_age_wt = ivm_cov_par*(exp(-ivm_min_age/21) - exp(-ivm_max_age/21))
ivm_cov = ivm_age_wt*prop_human_HR
...
output(ivm_cov_par) <- ivm_cov_par
