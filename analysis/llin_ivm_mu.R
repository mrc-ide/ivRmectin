...
year_period <- 3*365
y3_end_ivm <-  year_ivm_before_on + year_period
...
df_5_mean_EIR = df_5_epi %>% #UTN + ivm
  filter(between(t, year_ivm_before_on, y3_end_ivm)) %>%
  summarise(mean_EIR = mean(EIR_tot))  # 105.2722


df_0_mean_EIR - df_3_mean_EIR #37.82529

df_0_mean_EIR - df_5_mean_EIR # -35.86002
...
df_6_mean_EIR <- df_6_epi %>% #untreated nets and ivm
  filter(between(t, year_ivm_before_on, y3_end_ivm)) %>%
  summarise(mean_EIR = mean(EIR_tot)) # 104.5639

df_2_mean_EIR - df_6_mean_EIR #  -35.54429
...
#rel diff HS UTN
rel_EIR_UTN_HS <- ((df_0_mean_EIR-df_5_mean_EIR)/df_5_mean_EIR)*100 #-34.06411

#rel diff NC
rel_EIR_NC <- ((df_2_mean_EIR-df_4_mean_EIR)/df_2_mean_EIR)*100 # 54.50164

#rel diff NC untreated nets
rel_EIR_UTN_NC <- ((df_2_mean_EIR - df_6_mean_EIR)/df_2_mean_EIR)*100 #-51.49886
...
avhc_summary <- out.df %>%
  ...
  summarise(mean_avhc = mean(avhc),
            mean_mu = mean(mu)) %>%
  mutate(abs_diff_avhc  = 0.307 - mean_avhc,
         rel_diff_avhc = (0.307 - mean_avhc)/0.307,
...
plot_grid(abs_diff_params, abs_diff_params_UTN, rel_diff_params,  rel_diff_params_UTN,
