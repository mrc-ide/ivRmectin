...
itn_cov_vector <- seq(0, 1, 0.2)
ivm_cov_vector <- seq(0, 1, 0.2)
...
out_df <- do.call(rbind,
                  sapply(1:length(itn_cov_vector), function(x){
                    as.data.frame(res_out_list[[x]]) %>%
                      select(t, mv, mu, avhc, av_mosq_sum, itn_cov, ivm_cov, Sxtot, Extot, Ixtot, mvxtot, Sx_dead, Ex_dead, Ix_dead, mvx_dead) %>%
                      mutate(ref = x)
                  }, simplify = F))
...
#repeat for untreated nets scenario####
#studying the relationship between LLIN usage and IVM killing effect
...
out_lapply_list_UTN <- lapply(itn_cov_vector, create_ITN_cov_loop) #loop through all parameter values

res_out_list_UTN <- lapply(out_lapply_list_UTN, runfun) #put these values into the model

#now go through the res_out_list and save key parameters: av_mosq, LLIN cov, t, Sxtot, Extot, Ixtot
require(tidyverse)
out_df_UTN <- do.call(rbind,
                  sapply(1:length(itn_cov_vector), function(x){
                    as.data.frame(res_out_list_UTN[[x]]) %>%
                      select(t, mv, mu, avhc, av_mosq_sum, itn_cov, ivm_cov, Sxtot, Extot, Ixtot, mvxtot, Sx_dead, Ex_dead, Ix_dead, mvx_dead) %>%
                      mutate(ref = x)
                  }, simplify = F))

write.csv(out_df_UTN, file = "data/llin_ivm_muh/out_df_UTN.csv", row.names = FALSE)
...
out_df_2 <- do.call(rbind,
                  sapply(1:(length(itn_cov_vector)*length(ivm_cov_vector)), function(x){
                    as.data.frame(res_out_2_list[[x]]) %>%
                      select(t, mv, avhc, itn_cov, ivm_cov, Sxtot, Extot, Ixtot, mvxtot, FOIv, Sx_dead, Ex_dead, Ix_dead, mvx_dead, EIR_tot) %>%
                      mutate(ref = x)
                  }, simplify = F))
...
out_df_3 <- do.call(rbind,
                    sapply(1:(length(itn_cov_vector)*length(ivm_cov_vector)), function(x){
                      as.data.frame(res_out_3_list[[x]]) %>%
                        select(t, mv, avhc, av_mosq_sum, itn_cov, ivm_cov, Sxtot, Extot, Ixtot, mvxtot, FOIv, lag_FOIv, Sx_dead, Ex_dead, Ix_dead, mvx_dead, EIR_tot) %>%
                        mutate(ref = x)
                    }, simplify = F))
...
IG2_param_df_crit <- expand.grid(dn0_med = IG2_d_ITN0, ivm_cov = ivm_cov_vector) #606 rows
IG2_param_df <- left_join(IG2_param_df_crit,
                          IG2_nets_df %>% dplyr::select(dn0_med, rn0_med, resistance),
                          by = c("dn0_med")) %>%
  mutate(net_type = "IG2") %>%
  rename(d_ITN0 = dn0_med, r_ITN0 = rn0_med)

#repeat for pyrethroid nets
pyr_param_df_crit <- expand.grid(dn0_med = pyr_d_ITN0, ivm_cov = ivm_cov_vector)
pyr_param_df <- left_join(pyr_param_df_crit,
                          pyr_only_df %>% dplyr::select(dn0_med, rn0_med, resistance),
                          by = c("dn0_med")) %>%
  mutate(net_type = "pyrethroid only") %>%
  rename(d_ITN0 = dn0_med, r_ITN0 = rn0_med)
#repeat for pbo nets
pbo_param_df_crit <- expand.grid(dn0_med = pbo_d_ITN0, ivm_cov = ivm_cov_vector)
pbo_param_df <- left_join(pbo_param_df_crit,
                          pbo_nets_df %>% dplyr::select(dn0_med, rn0_med, resistance),
                          by = c("dn0_med")) %>%
...
IG2_out_4_list <- lapply(IG2_param_list, create_ivm_itn_type_vec) #putting param list into the function to generate parameter set
pyr_out_4_list <- lapply(pyr_param_list, create_ivm_itn_type_vec)
pbo_out_4_list <- lapply(pbo_param_list, create_ivm_itn_type_vec)
