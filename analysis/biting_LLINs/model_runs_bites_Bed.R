...
write.csv(antag_pyr_out_df_bites_Bed, file = "analysis/biting_LLINs/biting_antag_out_pyr_IVM_ITN_bites_Bed_df.csv", row.names = FALSE)
...
write.csv(add_pyr_out_df_bites_Bed, file = "analysis/biting_LLINs/biting_add_out_pyr_IVM_ITN_bites_Bed_df.csv", row.names = FALSE)
...
antag_pyr_out_list_LLIN <- lapply(pyr_param_list_bites_bed, antag_ITN_cov_loop_bites_Bed) #loop through all parameter values

res_antag_pyr_out_LLIN <- lapply(antag_pyr_out_list_LLIN, runfun) #put these values into the model
...
pyr_out_df_antag_LLIN_bites_Bed <- do.call(rbind, sapply(1:(length(itn_cov_vector)*length(species)*length(res_vector)),
                                               function(x){
                              as.data.frame(res_antag_pyr_out_LLIN[[x]]) %>%
...
write.csv(pyr_out_df_antag_LLIN_bites_Bed, file = "analysis/biting_LLINs/pyr_out_df_antag_LLIN_bites_Bed.csv", row.names = FALSE)
...
write.csv(pyr_out_df_add_LLIN_bites_Bed, file = "analysis/biting_LLINs/pyr_out_df_add_LLIN_bites_Bed.csv", row.names = FALSE)
