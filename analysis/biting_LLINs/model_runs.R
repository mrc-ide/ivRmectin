...
bites_Bed <- c("0.85", "0.71", "0.94", "0.21")
...
pyr_out_df <- do.call(rbind, sapply(1:(length(itn_cov_vector)*length(species)*length(res_vector)), function(x){
...
write.csv(pyr_out_df, file = "analysis/biting_LLINs/biting_antag_out_pyr_df.csv", row.names = FALSE)
...
add_pyr_out_list <- lapply(pyr_param_list, add_ITN_cov_loop) #loop through all parameter values

add_res_pyr_out <- lapply(add_pyr_out_list, runfun) #put these values into the model


add_pyr_out_df <- do.call(rbind, sapply(1:(length(itn_cov_vector)*length(species)*length(res_vector)), function(x){
  as.data.frame(add_res_pyr_out[[x]]) %>%
    select(t, mu, mv, avhc, itn_cov, EIR_tot, slide_prev0to5, bites_Bed,
           d_ITN0, r_ITN0, itn_loss, Q0) %>%
    mutate(ref = x, net_type = "pyr_only",
           species = case_when(bites_Bed == bites_Bed_vec[1] ~ "gambiae",
...
write.csv(add_pyr_out_df, file = "analysis/biting_LLINs/biting_add_out_pyr_df.csv", row.names = FALSE)
write.csv(add_pyr_pbo_out_df, file = "analysis/biting_LLINs/biting_add_out_pyr_pbo_df.csv", row.names = FALSE)
write.csv(add_IG2_out_df, file = "analysis/biting_LLINs/biting_add_out_IG2_df.csv", row.names = FALSE)
...
pyr_out_df_antag_LLIN <- do.call(rbind, sapply(1:(length(itn_cov_vector)*length(species)*length(res_vector)),
                                               function(x){
                              as.data.frame(res_pyr_out[[x]]) %>%
...
write.csv(pyr_out_df_antag_LLIN, file = "analysis/biting_LLINs/pyr_out_df_antag_LLIN.csv", row.names = FALSE)


#repeats for Q0 - keep phi-B fixed at the gambiae-like vector
...
#just going to change one thing at a time - keep Q0 high and explore multiple phi-Bs
#bites_Bed <- c("0.85", "0.71", "0.94", "0.21")
bites_Bed <- rep(0.85, 4) #for gambiae-like vector
biting_df <- data.frame(species, Q0, bites_Bed)

# for pyrethroid nets
pyr_param_df_crit_Q0 <- expand.grid(dn0_med = pyr_only_d_ITN0, itn_cov = itn_cov_vector, species = species)
pyr_param_df_1_Q0 <- left_join(pyr_param_df_crit_Q0,
                               df_pyr_only %>% dplyr::select(dn0_med, rn0_med, gamman_med, resistance),
                               by = c("dn0_med")) %>%
  mutate(net_type = "pyrethroid only") %>%
  mutate(gamman_med = gamman_med*365) %>%
...
pyr_param_df_Q0 <- left_join(pyr_param_df_1_Q0,
                             biting_df %>% dplyr::select(species, Q0, bites_Bed),
                             by = c("species"))


for (i in seq_len(nrow(pyr_param_df_Q0))){
  pyr_param_list_Q0[[i]] <- as.numeric(pyr_param_df_Q0[i,])
}

antag_ITN_cov_loop_Q0 <- function(itn_type_ivm_param){
  d_ITN0_in <- itn_type_ivm_param[1]
  itn_cov_in <-itn_type_ivm_param[2]
  bites_Bed_in<- itn_type_ivm_param[9]
  Q0_in <- itn_type_ivm_param[8]
...
pyr_out_list_Q0 <- lapply(pyr_param_list_Q0, antag_ITN_cov_loop) #loop through all parameter values

res_pyr_out_Q0 <- lapply(pyr_out_list_Q0, runfun) #put these values into the model

#bites_Bed_vec <- c(0.85, 0.8)



pyr_out_df_antag_LLIN_Q0 <- do.call(rbind, sapply(1:(length(itn_cov_vector)*length(species)*length(res_vector)),
                                               function(x){
                                                 as.data.frame(res_pyr_out_Q0[[x]]) %>%
                                                   select(t, mu, mv, avhc, itn_cov, EIR_tot, slide_prev0to5,
                                                          d_ITN0, r_ITN0, itn_loss, bites_Bed, Q0) %>%
                                                   mutate(ref = x, net_type = "pyr_only", model = "antag",
                                                          species = case_when(Q0 == Q0_vec[1] ~ "gambiae",
                                                                              Q0 == Q0_vec[2] ~ "arabiensis",
                                                                              Q0 == Q0_vec[3] ~ "funestus",
                                                                              Q0 == Q0_vec[4] ~ "stephensi",
                                                                              TRUE ~ NA_character_))
                                               }, simplify = F))

write.csv(pyr_out_df_antag_LLIN_Q0, file = "analysis/biting_LLINs/pyr_out_df_antag_LLIN_Q0.csv", row.names = FALSE)
