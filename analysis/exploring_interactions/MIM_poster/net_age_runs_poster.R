devtools::load_all()
#CHECK START TIMES, OUTPUT IS RIGHT
...
#CODE REQUIRES FURTHER PROCESSING TO ENSURE IVM_STARTS AND NET AGES ARE PROCESSED CORRECTLY
...
#ivm_parmscheck <- ivRmectin::ivm_fun(
#  IVM_start_times = c(3120, 3150, 3180), #distribution every 3 months
...
ivm_nets_starting <- list(ivm_nets_1) #JUST DOING 6 month nets
...
pyr_param_df2 <- readRDS("data/species_runs_net_age.rds")
head(pyr_param_df2)

#let's just filter to one scenario with gambiae and stephensi

res <- unique(pyr_param_df2$d_ITN0)

pyr_param_df2 <- pyr_param_df2 %>%
  filter(itn_cov %in% c(0.2,0.8) & d_ITN0 %in% c(res[1], res[5]) & init_EIR == "25") #filtering to the EIR of 25
...
ivm_parms1 <- ivRmectin::ivm_fun(#IVM_start_times = c(3120, 3150, 3180), #distribution every 3 months
  IVM_start_times = IVM_start1, #JUST DOING when nets are 6months old
  time_period = time_period,
  hazard_profile = ivm_haz$IVM_300_3_HS[1:23],
  #hazard_profile = hazzy,
  ivm_coverage=ivm_cov,
...
#x <- rep(ivm_nets_starting, nrow(pyr_param_df2))
#y <- rep(pyr_param_list, 3)

#MODELS WITH IVM + LLIN
...
my_sim_antag_ITN <- function(){
  ...
  pyr_out_df_antag_ITN <- do.call(rbind, sapply(1:(nrow(pyr_param_df2)), function(x){
    df <- as.data.frame(res_pyr_out_antag_ITN[[x]])
    df2 <- as.data.frame(dplyr::select(.data = df,t, mu, mv, avhc, itn_cov, EIR_tot, EIRout, slide_prev0to5,
                                       d_ITN0, r_ITN0, itn_loss, bites_Bed, Q0, IVRM_sr, s_ITN, d_ITN, r_ITN, clin_inc0to5))
...
my_sim_antag_ITN_IVM <- function(){
  ...
  pyr_out_df_antag_ITN_IVM <- do.call(rbind, sapply(1:(nrow(pyr_param_df2)), function(x){
...
my_sim_add_ITN <- function(){
  ...
  pyr_out_df_add_ITN <- do.call(rbind, sapply(1:(nrow(pyr_param_df2)), function(x){
...
my_sim_add_ITN_IVM <- function(){
  ...
  pyr_out_df_add_ITN_IVM <- do.call(rbind, sapply(1:(nrow(pyr_param_df2)), function(x){
...
df_list <- list(antag_ITN, antag_ITN_IVM, add_ITN, add_ITN_IVM)

fun_species <- function(df) {
  df_out <- df %>%
    mutate(species = case_when(bites_Bed == 0.85 & Q0 == 0.92 ~ "gambiae",
                                                            bites_Bed == 0.8 & Q0 == 0.71 ~ "arabiensis",
                                                            bites_Bed == 0.78 & Q0 == 0.94 ~ "funestus",
...
processed_df <- lapply(df_list, fun_species)
names(processed_df) <- c("antag_LLIN", "antag_LLIN_IVM", "add_LLIN", "add_LLIN_IVM")
list2env(processed_df,.GlobalEnv)

#save outputs
saveRDS(antag_LLIN, file = "analysis/exploring_interactions/MIM_poster/antag_LLIN_6m_MDA.rds")
saveRDS(antag_LLIN_IVM, file = "analysis/exploring_interactions/MIM_poster/antag_LLIN_IVM_6m_MDA.rds")
saveRDS(add_LLIN, file = "analysis/exploring_interactions/MIM_poster/add_LLIN_6m_MDA.rds")
saveRDS(antag_LLIN_IVM, file = "analysis/exploring_interactions/MIM_poster/add_LLIN_IVM_6m_MDA.rds")

#bound antag models
antag_LLIN <- antag_LLIN %>%
  mutate(model = "antag_LLIN")
antag_LLIN_IVM <- antag_LLIN_IVM %>%
  mutate(model = "antag_LLIN_IVM")
add_LLIN <- add_LLIN %>%
  mutate(model = "add_LLIN")
add_LLIN_IVM <- add_LLIN_IVM %>%
  mutate(model = "add_LLIN_IVM")

antag <- rbind(antag_LLIN, antag_LLIN_IVM)
add <- rbind(add_LLIN, add_LLIN_IVM)
saveRDS(antag, file = "analysis/exploring_interactions/MIM_poster/antag.rds")
saveRDS(add, file = "analysis/exploring_interactions/MIM_poster/add.rds")
