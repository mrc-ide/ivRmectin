...
itn_cov_vector <- c(0, 0.2, 0.6, 0.8)
res_vector <- rep(0,4) #no resistance in the sens analysis
init_EIR_vec <- c(2, 25, 100)
...
pyr_param_df <- pyr_param_df %>%
  filter(resistance == 0.0, init_EIR == 25) %>%
  select(-resistance) %>%
  distinct()
head(pyr_param_df)
...
saveRDS(antag, file = "analysis/exploring_interactions/MIM_poster/itn_cov/antag.rds")
saveRDS(add, file = "analysis/exploring_interactions/MIM_poster/itn_cov/add.rds")
