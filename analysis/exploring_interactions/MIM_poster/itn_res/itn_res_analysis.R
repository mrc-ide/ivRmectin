#analysis on other parameters: Q0 and resistance
...
Q0_vector <-       rep(0.95, 4)
itn_cov_vector <- rep(0.8, 4)
res_vector <- c(0, 0.2, 0.6, 0.8) #no resistance in the sens analysis

#when holding other parameters constant, it is to maximise efficacy of nets
...
lapply(filenames, function(x) {
  df <- subset(read.csv(x), resistance == 0 | resistance  == 0.2 | resistance == 0.6 | resistance == 0.8) #4 resistance scenarios
  return(df)
}) -> list_data
...
pyr_param_df <- left_join(pyr_param_df_crit,
  ...
  mutate(gamman_med = gamman_med*365, bites_Bed = 0.95, Q0 = 0.95) %>%
  rename(d_ITN0 = dn0_med, r_ITN0 = rn0_med, itn_half_life = gamman_med)
head(pyr_param_df)

#filter to make it really simple
cov <- itn_cov_vector[1]
pyr_param_df <- pyr_param_df %>%
  filter(init_EIR == 25 & itn_cov == cov) %>%
  distinct() %>%
  select(-resistance)
head(pyr_param_df)

unique(pyr_param_df$init_EIR)
unique(pyr_param_df$Q0)
unique(pyr_param_df$d_ITN0)
unique(pyr_param_df$r_ITN0)
...
antag_ITN_cov_loop <- function(itn_type_ivm_param){
  ...
  bites_Bed_in <- itn_type_ivm_param[6]
...
unique(antag_ITN$ref)
unique(antag_ITN$Q0)
unique(antag_ITN$bites_Bed)
unique(antag_ITN$d_ITN0)
unique(antag_ITN$itn_cov)
...
#save outputs
#saveRDS(antag_ITN, file = "analysis/exploring_interactions/MIM_poster/bites_Bed/antag_LLIN_6m_MDA.rds")
#saveRDS(antag_ITN_IVM, file = "analysis/exploring_interactions/MIM_poster/bites_Bed/antag_LLIN_IVM_6m_MDA.rds")
#saveRDS(add_ITN, file = "analysis/exploring_interactions/MIM_poster/bites_Bed/add_LLIN_6m_MDA.rds")
#saveRDS(antag_ITN_IVM, file = "analysis/exploring_interactions/MIM_poster/bites_Bed/add_LLIN_IVM_6m_MDA.rds")
...
saveRDS(antag, file = "analysis/exploring_interactions/MIM_poster/itn_res/antag.rds")
saveRDS(add, file = "analysis/exploring_interactions/MIM_poster/itn_res/add.rds")
