...
dynamics_plot_bb <- ggplot(mods_dynamics, aes(x = t/365, y = slide_prev0to5*100, linetype = as.factor(model), col = as.factor(bites_Bed)))+
  ...
  annotate(geom = "text", x = (net_seq[1]/365)+3, y = 55, label = "LLIN distribution (every 3 years)", col = "#1f78b4", size = 6)+
  annotate(geom = "text", x = (IVM_start1[1]/365)+(420/365), y = 52, label = "Endectocide MDA", size = 6)+
...
efficacy_plot_abs_EIR_bb <- ggplot(rel_red_EIR, aes(x = as.factor(bites_Bed), y = abs_EIR))+
  geom_bar(stat = "identity", fill = "white", col = "black")+
  theme_minimal()+
  #scale_colour_manual(values = res_pals, labels = c("90% resistance", "No resistance"),
  #                    name = "Pyrethroid resistance")+
...
model_comparison_plot_EIR_bb <- ggplot(model_compare_EIR, aes(x = abs_EIR_antag, y = abs_EIR_add, col = as.factor(bites_Bed), group = 1))+
  geom_point(alpha = 0.8)+
  scale_colour_manual(values = sp_pals, labels = c("0.25", "0.5", "0.75", "0.9"),
                      name = "Proportion of bites in bed")+
  #scale_size_manual(values = c(3, 5), labels = c("20%", "80%"), name = "LLIN coverage")+
