...
#look at influence of timing of MDA in relation to ITN campaign, additionally with variability in ITN usage.
time_period <- 365*15
mda_int <- 30
...
df_var1a <- df_var1
df_var1_new <- df_var1a
df_var1_new$itn_cov <- 0.3
df_var1 <- rbind(df_var1a, df_var1_new)
df_var1_new2 <- df_var1a
df_var1_new2$itn_cov <- 0.7
df_var1 <- rbind(df_var1, df_var1_new2)

saveRDS(df_var1, file = "analysis/exploring_interactions/malariasim-odin/scenario-parameter-set-mda-times-itn-covs.rds")

#one row#one rowdf_var1_new2
#df_var1 <- df_var1[1,]
...
my_sim_mod1 <- function(){
  ...
  mod1_df <- do.call(rbind, sapply(1:(nrow(df_var1)), function(x){
    ...
    df2 <-  as.data.frame(dplyr::select(.data = df, t, mu, mv, mvx_dead,Q0, ivm_cov, slide_prev0to5, EIRout, Ivtot, IVRM_sr, bites_Bed, clin_inc0to5, itn_cov))
...
saveRDS(df_var1_mda_early, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/df_var1_mda_early_itn_covs.rds")
...
saveRDS(df_var1_mda_medium, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/df_var1_mda_medium_itn_covs.rds")
...
saveRDS(df_var1_mda_late, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/df_var1_mda_late_itn_covs.rds")
...
df_baseline <- df_baseline %>%
  mutate(bites_Bed = 0.95, ivm_cov = 0.7, itn_cov = 0) #to help binding
...
df_ITN <- my_sim_mod5()
saveRDS(df_ITN, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/df_ITN_itn_covs.rds")

df_baseline <- readRDS("analysis/exploring_interactions/MIM_poster/chapter_plots/df_baseline.rds") %>%
  mutate(bites_Bed = 0.95, ivm_cov = 0.7, itn_cov = 0) #to help binding

df_ITN <- readRDS("analysis/exploring_interactions/MIM_poster/chapter_plots/df_ITN_itn_covs.rds")
df_var1_mda_early <- readRDS("analysis/exploring_interactions/MIM_poster/chapter_plots/df_var1_mda_early_itn_covs.rds")
df_var1_mda_medium<- readRDS("analysis/exploring_interactions/MIM_poster/chapter_plots/df_var1_mda_medium_itn_covs.rds")
df_var1_mda_late<- readRDS("analysis/exploring_interactions/MIM_poster/chapter_plots/df_var1_mda_late_itn_covs.rds")
...
df_timings %>%
  ...
  summarise(EIRout = EIRout,
            itn_cov = itn_cov) #ref 2 is the high EIR

#ref 2,4,6 are high EIR and each of the ITN coverages
...
dynamics_timing_mda <- df_timings %>%
  ...
  coord_cartesian(ylim = c(0,100), xlim = c(-0.5, 7.5))+
  xlab("Time (years) since first ITN campaign")+
  ylab("Slide prevalence (%) \n in under 5-years-olds")+
...
impact_baseline_early <- df_baseline %>%
  filter(between(t, IVM_begin1, IVM_begin1+180) & ref %in% c(2,4,6)) %>%
  group_by(ref) %>%
...
impact_baseline_medium <- df_baseline %>%
  filter(between(t, IVM_begin2, IVM_begin2+180) & ref %in% c(2,4,6)) %>% #choose same times, in case add on seasonal profiles
  group_by(ref) %>%
  mutate(clin_inc0to5 = clin_inc0to5*1000) %>%
  summarise(tot_cases_baseline = sum(clin_inc0to5))

impact_medium_MDA <- df_var1_mda_medium %>%
  filter(between(t, IVM_begin2, IVM_begin2+180)& ref %in% c(2,4,6)) %>%
  group_by(ref) %>%
  mutate(clin_inc0to5 = clin_inc0to5*1000) %>%
...
impact_baseline_late <- df_baseline %>%
  filter(between(t, IVM_begin3, IVM_begin3+180)& ref %in% c(2,4,6)) %>% #choose same times, in case add on seasonal profiles
  group_by(ref) %>%
  mutate(clin_inc0to5 = clin_inc0to5*1000) %>%
  summarise(tot_cases_baseline = sum(clin_inc0to5))

impact_late_MDA <- df_var1_mda_late %>%
  filter(between(t, IVM_begin3, IVM_begin3+180)& ref %in% c(2,4,6)) %>%
  group_by(ref) %>%
  mutate(clin_inc0to5 = clin_inc0to5*1000) %>%
...
df_wide <- output_impact_rel_baseline %>%
  mutate(
    case_type = case_when(
      ref == 2 ~ "case_avert_rel",
      ref == 4 ~ "case_avert_rel_lower",
      ref == 6 ~ "case_avert_rel_upper"
    )
  ) %>%
  select(-ref) %>%
  pivot_wider(
    names_from = case_type,
    values_from = case_avert_rel
  )
...
case_avert_plot <- output_impact_rel_baseline %>%
  mutate(
    scenario = factor(
      ...
      labels = c("6 months", "1 year", "2 years")
    ),
    ref = factor(
      ref,
      levels = c(4, 2, 6),   # order facets
  ...
  scale_pattern_manual(
    values = c("none", "stripe"), name = "Efficacy measurement",
    labels = c(
      "Relative to baseline (no interventions) scenario",
      "Relative to scenario with historic use of ITNs"
    )
  ) +
  facet_wrap(vars(ref)) +
  # geom_errorbar(aes(ymin = case_avert_rel_upper, ymax = case_avert_rel_lower),
  #               position = position_dodge()) +
  theme_bw(base_size = 14) +
...
mda_timing_plot_SM <- cowplot::plot_grid(dynamics_timing_mda, case_avert_plot,
                                           labels = c("A", "B"))



ggsave(mda_timing_plot_SM, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/mda_timing_plot_SM_ITN_cov.pdf")




#for just ITN cov 50%
dynamics_timing_mda_itn50 <- df_timings %>%
  filter(ref ==2) %>%
  #filter(ref %in% c(2,4,6)) %>%
  mutate(model_type = factor(model_type,
                             levels = c("baseline", "ITN-int", "hazards-early-mda",
...
case_avert_plot_itn50 <- output_impact_rel_baseline %>%
  mutate(
    scenario = factor(
      scenario,
      levels = c("early-MDA", "medium-MDA", "late-MDA"),
...
mda_timing_plot_fig2 <- cowplot::plot_grid(dynamics_timing_mda_itn50,
                                           case_avert_plot_itn50, labels = c("A", "B"))
