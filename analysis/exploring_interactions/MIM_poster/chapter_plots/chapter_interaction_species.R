#manuscript: interaction between ITNs and endectocides

#explore for each of species: arabiensis, funestus, gambiae and stephensi
#in high transmission setting
#ivm distribution has 6 months after the most recent ITN distribution

#figure 1: dynamics in prevalence and EIR


#figure 2: efficacy - epidemiological impact
#A: measured as rel reduction in prevalence 4 weeks after last MDA
#B: measured as cases averted in U5 from start of MDA to 6m later

#figure 3:
#A)efficacy as predicted by both models
#B)avhc and mu plot
...
dynamics_plot_prev <- ggplot(mods_dynamics, aes(x = (t-start_ITN)/365, y = slide_prev0to5*100, linetype = as.factor(model), col = as.factor(species)))+
  ...
  ylim(0, 60)+
  annotate("segment", x = (IVM_start1[1]-start_ITN)/365, xend = (IVM_start1[1]-start_ITN)/365, y = 33, yend = 31, colour = "black", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (IVM_start1[2]-start_ITN)/365, xend = (IVM_start1[2]-start_ITN)/365, y = 33, yend = 31, colour = "black", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (IVM_start1[3]-start_ITN)/365, xend = (IVM_start1[3]-start_ITN)/365, y = 33, yend = 31, colour = "black", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[1]-start_ITN)/365, xend = (net_seq[1]-start_ITN)/365, y = 45, yend = 41, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[2]-start_ITN)/365, xend = (net_seq[2]-start_ITN)/365, y = 35, yend = 31, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[3]-start_ITN)/365, xend = (net_seq[3]-start_ITN)/365, y = 35, yend = 31, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[4]-start_ITN)/365, xend = (net_seq[4]-start_ITN)/365, y = 35, yend = 31, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate(geom = "text", x = (net_seq[1]/365)+2, y = 47, label = "ITN distribution (every 3 years)", col = "#1f78b4", size = 6)+
  ...
  annotate("rect", fill = "yellow", alpha = 0.5,
           xmin = (IVM_start1[1]-start_ITN)/365,
                   xmax = ((IVM_start1[3]-start_ITN)/365)+(23/365),
           ymin = -Inf, ymax = 30)

dynamics_plot_EIR <- ggplot(mods_dynamics, aes(x = (t-start_ITN)/365, y = EIRout*1000, linetype = as.factor(model), col = as.factor(species)))+
  geom_line(linewidth = 1)+
  theme_minimal()+
  labs(y = "Daily EIR (bites per 1000 persons per day)",
       x = "Time (years)")+
  coord_cartesian(ylim = c(0, 90))+
  scale_linetype_manual(values = lines, name = "Intervention", labels = c("ITN", "LLIN & endectocide"))+
  annotate("segment", x = (IVM_start1[1]-start_ITN)/365, xend = (IVM_start1[1]-start_ITN)/365, y = 35, yend = 31, colour = "black", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (IVM_start1[2]-start_ITN)/365, xend = (IVM_start1[2]-start_ITN)/365, y = 35, yend = 31, colour = "black", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (IVM_start1[3]-start_ITN)/365, xend = (IVM_start1[3]-start_ITN)/365, y = 35, yend = 31, colour = "black", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[1]-start_ITN)/365, xend = (net_seq[1]-start_ITN)/365, y = 65, yend = 61, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[2]-start_ITN)/365, xend = (net_seq[2]-start_ITN)/365, y = 35, yend = 31, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[3]-start_ITN)/365, xend = (net_seq[3]-start_ITN)/365, y = 35, yend = 31, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate("segment", x = (net_seq[4]-start_ITN)/365, xend = (net_seq[4]-start_ITN)/365, y = 35, yend = 31, colour = "#1f78b4", arrow = arrow(length = unit(0.01, "npc")))+
  annotate(geom = "text", x = ((net_seq[1]-start_ITN)/365)+2, y = 67, label = "ITN distribution (every 3 years)", col = "#1f78b4", size = 6)+
  annotate(geom = "text", x = ((IVM_start1[1]-start_ITN)/365)+(420/365), y = 38, label = "Endectocide MDA", size = 6)+
  scale_colour_manual(name = "Anopheles species", labels = c("arabiensis",
                                                             "funestus",
                                                             "gambiae",
                                                             "stephensi"),
...
figure_1_dynamics <- cowplot::plot_grid(dynamics_plot_prev, dynamics_plot_EIR, ncol = 2,
                   labels = c("A", "B"))
...
cowplot::plot_grid(efficacy_species_plot, efficacy_species_plot_abs)
#ggsave(efficacy_species_plot_abs, file = "analysis/exploring_interactions/MIM_poster/eff_sp_plot_abs.svg")

#cases averted plot
inc_6m <- IVM_start1[1]+180
rel_red_cases <- mods %>%
  #filter(between(t, IVM_start1[1], IVM_start3[3]+23)) %>% #look in ivermectin kill time only
  #filter(d_ITN0 == res[1] & itn_cov == 0.8 & model %in% c("antag_LLIN", "antag_LLIN_IVM")) %>%
  filter(between(t, IVM_start1[1], inc_6m)) %>% #from start of MDA to 6m later
  filter(itn_cov == 0.8 & model %in% c("antag_LLIN", "antag_LLIN_IVM")) %>%
  select(t, d_ITN0, species, clin_inc0to5, model) %>%
  spread(key = model, value = clin_inc0to5) %>%
  group_by(species, d_ITN0) %>%
  summarise(tot_cases_antag_LLIN = sum(antag_LLIN),
            tot_antag_LLIN_IVM = sum(antag_LLIN_IVM)) %>%
  mutate(red_inc = ((tot_cases_antag_LLIN - tot_antag_LLIN_IVM)/tot_cases_antag_LLIN)*100,
         abs_inc = tot_cases_antag_LLIN - tot_antag_LLIN_IVM)

res_pals <- c("#1f78b4", "#33a02c")

no_res_cases <- rel_red_cases %>%
  filter(d_ITN0 == res[1])


efficacy_species_plot_cases <- ggplot(no_res_cases, aes(x = species, y = red_inc))+
  geom_bar(stat = "identity", fill = "white", col = "black")+
  theme_minimal()+
  #scale_colour_manual(values = res_pals, labels = c("90% resistance", "No resistance"),
  #                    name = "Pyrethroid resistance")+
  ylab("Relative reduction in clinical incidence in under 5s (%) \n due to endectocide")+
  xlab("Anopheles species")

#ggsave(efficacy_species_plot, file = "analysis/exploring_interactions/MIM_poster/eff_sp_plot.svg")

#absolute drop is very very small
efficacy_species_plot_abs <- ggplot(no_res_cases, aes(x = species, y = abs_inc))+
  geom_bar(stat = "identity", fill = "white", col = "black")+
  theme_minimal()+
  #scale_colour_manual(values = res_pals, labels = c("90% resistance", "No resistance"),
  #                    name = "Pyrethroid resistance")+
  ylab("Absolute reduction in clinical incidence in under 5s \n due to endectocide")+
  xlab("Anopheles species")

efficacy_plot_cases_averted <- cowplot::plot_grid(efficacy_species_plot_cases, efficacy_species_plot_abs)
...
model_compare_inc <- mods %>%
  filter(between(t, IVM_start1[1], inc_6m)) %>% #from start of MDA to 6m later
  #filter(d_ITN0 == res[1] & itn_cov == 0.8 & model %in% c("antag_LLIN", "antag_LLIN_IVM")) %>%
  #filter(itn_cov == 0.8 & model %in% c("antag_LLIN", "antag_LLIN_IVM")) %>%
  select(t, d_ITN0, itn_cov, species, clin_inc0to5, model) %>%
  spread(key = model, value = clin_inc0to5) %>%
  group_by(species, d_ITN0, itn_cov) %>%
  summarise(tot_cases_antag_LLIN = sum(antag_LLIN),
            tot_cases_antag_LLIN_IVM = sum(antag_LLIN_IVM),
            tot_cases_add_LLIN = sum(add_LLIN),
            tot_cases_add_LLIN_IVM = sum(add_LLIN_IVM)) %>%
  mutate(red_inc_antag = ((tot_cases_antag_LLIN - tot_cases_antag_LLIN_IVM)/tot_cases_antag_LLIN)*100,
         abs_inc_antag = tot_cases_antag_LLIN - tot_cases_antag_LLIN_IVM,
         red_inc_add = ((tot_cases_add_LLIN - tot_cases_add_LLIN_IVM)/tot_cases_add_LLIN)*100,
         abs_inc_add = tot_cases_add_LLIN - tot_cases_add_LLIN_IVM)

lm_models <- lm(red_inc_add ~ red_inc_antag, data = model_compare_inc)
summary(lm_models) #adj r2 0.9688...
adjusted_r_squared <- summary(lm_models)$adj.r.squared

model_comparison_plot_inc <- ggplot(model_compare_inc, aes(x = red_inc_antag,
                                                             y = red_inc_add,
                                                             col = as.factor(species),
                                                             shape = as.factor(d_ITN0),
                                                             size = as.factor(itn_cov),
  ...
  scale_shape_manual(values = c(19, 17), labels = c("90% resistance", "No resistance"),
                     name = "Pyrethroid resistance")+
  labs(x = "Cases averted in under 5s (%) due to endectocide (model A)", y = "Cases averted (%) in under 5s due to endectocide (model B)")+
  theme_bw(base_size = 14)+
  theme(legend.position = c(0.8, 0.4))+
  coord_cartesian(xlim = c(0, 50), ylim = c(0, 50))+
  geom_smooth(method="lm", se = FALSE, show.legend = FALSE, lty = "dashed")+
  annotate("text", label = "italic(R)^2 == 0.9688", parse = TRUE, x =20, y = 30, col = "blue", size = 6)
