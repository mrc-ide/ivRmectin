...
model_compare_EIR <- mods %>%
  filter(between(t, IVM_start1[1], IVM_start1[3]+180)) %>%
...
model_comparison_plot_abs_EIR_bb <- ggplot(model_compare_EIR, aes(x = abs_EIR_antag, y = abs_EIR_add, fill = as.factor(bites_Bed), group = 1))+
  ...
  labs(x = "Absolute reduction in EIR \n due to endectocide (model A)", y = "Absolute reduction in EIR \n due to endectocide (model B)")+
  theme_minimal()+
  theme(legend.position = c(0.4, 0.6))+
  geom_smooth(method="lm", se = FALSE, show.legend = FALSE, lty = "dashed", col = "grey", alpha = 0.5)+
  annotate("text", label = paste0("italic(R)^2 ==",EIR_Rsq), parse = TRUE, x = 10, y = 17.5, col = "red", size = 6)+
  theme(axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12),
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
heatmap_Q0_bb <- ggplot(output_bb_Q0, aes(x = as.factor(bites_Bed), y = as.factor(Q0), fill = rel_diff_IVM))+
  geom_tile()+
  theme_bw()+
...
heatmap_bb_cov <- ggplot(output_bb_cov, aes(x = as.factor(bites_Bed), y = as.factor(itn_cov), fill = rel_diff_IVM))+
  geom_tile()+
  theme_bw()+
  scale_fill_viridis_c(limits = common_limits, name = "Cases averted (%) in \n under 5-year-olds due to endectocide")+
  xlab("Proportion of bites taken on people when they are in bed")+
...
heatmap_bb_res <- ggplot(output_bb_res, aes(x = as.factor(bites_Bed), y = as.factor(resistance), fill = rel_diff_IVM))+
  geom_tile()+
  theme_bw()+
  scale_fill_viridis_c(limits = common_limits, name = "Cases averted (%) in \n under 5-year-olds due to endectocide")+
  xlab("Proportion of bites taken on people when they are in bed")+
