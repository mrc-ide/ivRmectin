...
plots_early <- lapply(vars, function(v) {
  if (v == "EIRout") {
    p <- ggplot(high_eir_odin_mda_early, aes(x = t, y = .data[[v]] * 1000)) +
      geom_line() +
      geom_line(data = msim_early_bestfit, aes(x = t, y = .data[[v]] * 1000), col = "red") +
      labs(title = paste0(v, " (per 1000 persons)"))+
      theme_bw()
  } else {
    p <- ggplot(high_eir_odin_mda_early, aes(x = t, y = .data[[v]])) +
      geom_line() +
      geom_line(data = msim_early_bestfit, aes(x = t, y = .data[[v]]), col = "red") +
      labs(title = v)+
      theme_bw()
...
plots_medium <- lapply(vars, function(v) {
  if (v == "EIRout") {
    p <- ggplot(high_eir_odin_mda_medium, aes(x = t, y = .data[[v]] * 1000)) +
      geom_line() +
      geom_line(data = msim_medium_bestfit, aes(x = t, y = .data[[v]] * 1000), col = "red") +
      labs(title = paste0(v, " (per 1000 persons)"))+
      theme_bw()
  } else {
    p <- ggplot(high_eir_odin_mda_medium, aes(x = t, y = .data[[v]])) +
      geom_line() +
      geom_line(data = msim_medium_bestfit, aes(x = t, y = .data[[v]]), col = "red") +
      labs(title = v)+
      theme_bw()
...
plots_late <- lapply(vars, function(v) {
  if (v == "EIRout") {
    p <- ggplot(high_eir_odin_mda_late, aes(x = t, y = .data[[v]] * 1000)) +
      geom_line() +
      geom_line(data = msim_late_bestfit, aes(x = t, y = .data[[v]] * 1000), col = "red") +
      labs(title = paste0(v, " (per 1000 persons)"))+
      theme_bw()
  } else {
    p <- ggplot(high_eir_odin_mda_late, aes(x = t, y = .data[[v]])) +
      geom_line() +
      geom_line(data = msim_late_bestfit, aes(x = t, y = .data[[v]]), col = "red") +
      labs(title = v)+
      theme_bw()
...
ggplot(mda_timing_df_long, aes(x = factor(timing_mda, levels = c("early", "medium", "late")), y  = eff, fill = as.factor(model)))+
  ...
  ylim(0,100)+
  scale_fill_manual(name = "Model", values = c('#1b9e77','#d95f02'), labels = c("Odin-malariasim",
                                                                                "Odin"))
