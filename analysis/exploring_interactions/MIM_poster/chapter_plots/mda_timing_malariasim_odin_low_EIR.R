...
#working out the correct endec_mu and wane values for malariasim_exp_decay.R in low EIR setting
...
low_EIR <- unique(df_var1$init_EIR)[1]
...
time_odin_mda_early <- odin_mda_early %>%
  filter(ref == 1 & between(t, early_t_start, early_t_end)) %>%
  select(t,Ivtot)
...
time_msim_mda_early <- msim_mda_early %>%
  filter(ref >= 1 & ref <= 121 & between(t, early_t_start, early_t_end)) %>%
  select(t,Ivtot, ref)
...
time_odin_mda_medium <- odin_mda_medium %>%
  filter(ref == 1 & between(t, medium_t_start, medium_t_end)) %>%
  select(t,Ivtot)
...
time_msim_mda_medium <- msim_mda_medium %>%
  filter(ref >= 1 & ref <= 121 & between(t, medium_t_start, medium_t_end)) %>%
  select(t,Ivtot, ref)
...
time_odin_mda_late <- odin_mda_late %>%
  filter(ref == 1 & between(t, late_t_start, late_t_end)) %>%
  select(t,Ivtot)
...
time_msim_mda_late <- msim_mda_late %>%
  filter(ref >= 1 & ref <= 121 & between(t, late_t_start, late_t_end)) %>%
  select(t,Ivtot, ref)
...
msim_early_bestfit <- msim_mda_early %>%
  filter(ref >= 1 & ref <= 121 & endec_mu == endec_mu_vec[10] & wane == wane_vec[2])
...
low_eir_odin_mda_early <- odin_mda_early %>%
  filter(ref == 1)

low_eir_odin_mda_medium <- odin_mda_medium %>%
  filter(ref == 1)

low_eir_odin_mda_late <- odin_mda_late %>%
  filter(ref == 1)
...
plots_early <- lapply(vars, function(v) {
  if (v == "EIRout") {
    p <- ggplot(low_eir_odin_mda_early, aes(x = (t-net_seq[1])/365, y = .data[[v]] * 1000), col = mod_pals[1]) +
    ...
    p <- ggplot(low_eir_odin_mda_early, aes(x = (t-net_seq[1])/365, y = .data[[v]]*100), col = mod_pals[1]) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
      geom_line(data = msim_early_bestfit, aes(x = (t-net_seq[1])/365, y = .data[[v]]*100, col = "Model C"), alpha = 0.7, linewidth = 0.8) +
      labs(y = "Slide prevalence (%) \n in under 5-year-olds", x = "Time since first ITN campaign (years)") +
      scale_colour_manual(values = c("Model B1" = mod_pals[1],
                                     "Model C" = mod_pals[2]),
                                     name = "Model")+
      guides(col = "none")+
      theme_bw() +
      coord_cartesian(xlim = c(-0.5, 7.5), ylim = c(0,60))

  } else if (v == "Ivtot") {
    p <- ggplot(low_eir_odin_mda_early, aes(x = (t-net_seq[1])/365, y = .data[[v]]), col = mod_pals[1]) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
    ...
    p <- ggplot(low_eir_odin_mda_early, aes(x = (t-net_seq[1])/365, y = .data[[v]])) +
      ggeom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
      geom_line(data = msim_early_bestfit, aes(x = (t-net_seq[1])/365, y = .data[[v]], col = "Model C"), alpha = 0.7, linewidth = 0.8) +
      scale_colour_manual(values = c("Model B1" = mod_pals[1],
                                     "Model C" = mod_pals[2]),
                                     name = "Model")+
...
early_fits <- cowplot::plot_grid(plotlist = plots_early, labels = c("A","B","C","D"))

ggsave(early_fits,file = "analysis/exploring_interactions/MIM_poster/chapter_plots/early_fits_odin_msimodin_low_eir.pdf")


#medium
plots_medium <- lapply(vars, function(v) {
  if (v == "EIRout") {
    p <- ggplot(low_eir_odin_mda_medium, aes(x = (t-net_seq[1])/365, y = .data[[v]] * 1000)) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
    ...
    p <- ggplot(low_eir_odin_mda_medium, aes(x = (t-net_seq[1])/365, y = .data[[v]]*100)) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
    ...
    p <- ggplot(low_eir_odin_mda_medium, aes(x = (t-net_seq[1])/365, y = .data[[v]])) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
...
medium_fits <- cowplot::plot_grid(plotlist = plots_medium, labels = c("A","B","C","D"))
ggsave(medium_fits,file = "analysis/exploring_interactions/MIM_poster/chapter_plots/medium_fits_odin_msimodin_low_eir.pdf")

#late
plots_late <- lapply(vars, function(v) {
  if (v == "EIRout") {
    p <- ggplot(low_eir_odin_mda_late, aes(x = (t-net_seq[1])/365, y = .data[[v]] * 1000)) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
    ...
    p <- ggplot(low_eir_odin_mda_late, aes(x = (t-net_seq[1])/365, y = .data[[v]]*100)) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
    ...
    p <- ggplot(low_eir_odin_mda_late, aes(x = (t-net_seq[1])/365, y = .data[[v]])) +
      geom_line(aes(col = "Model B1"), alpha = 0.5, linewidth = 1) +
...
ggsave(late_fits,file = "analysis/exploring_interactions/MIM_poster/chapter_plots/late_fits_odin_msimodin_low_eir.pdf")

ggplot(low_eir_odin_mda_late, aes(x = t, y = clin_inc0to5 * 1000)) +
  geom_line() +
...
low_eir_msim_baseline <- msim_baseline %>%
  filter(ref == 1)

ggplot(low_eir_msim_baseline, aes(x = t, y = Ivtot))+
  geom_line()



baseline_Ivtot <-low_eir_msim_baseline %>%
...
low_eir_odin <- odin_baseline %>%
  filter(ref == 1 & between(t, late_t_start, late_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_baseline = Ivtot)

low_eir_odin_late <- odin_mda_late %>%
  filter(ref ==1 & between(t, late_t_start, late_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_int = Ivtot)

late_mda_comp_odin <- left_join(low_eir_odin, low_eir_odin_late)
...
baseline_prev <-low_eir_msim_baseline %>%
...
#compared to odin model
low_eir_odin_prev <- odin_baseline %>%
  filter(ref == 1 & t == late_t_end+30) %>%
  select(t, slide_prev0to5) %>%
  rename(prev_baseline = slide_prev0to5)

low_eir_odin_late_prev <- odin_mda_late %>%
  filter(ref == 1 & t == late_t_end+30) %>%
  select(t, slide_prev0to5) %>%
  rename(prev_int = slide_prev0to5)

late_mda_comp_odin <- left_join(low_eir_odin_prev, low_eir_odin_late_prev)
...
baseline_inc <- low_eir_msim_baseline %>%
  filter(between(t, late_t_start, late_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
...
#compared to odin model
low_eir_odin_inc <- odin_baseline %>%
  filter(ref == 1 & between(t, late_t_start, late_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
  rename(inc_baseline = clin_inc0to5)

low_eir_odin_late_inc <- odin_mda_late %>%
  filter(ref ==1 & between(t, late_t_start, late_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
  rename(inc_int = clin_inc0to5)

late_mda_comp_odin <- left_join(low_eir_odin_inc, low_eir_odin_late_inc)
...
medium_baseline_Ivtot <-low_eir_msim_baseline %>%
  filter(between(t, medium_t_start, medium_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_baseline = Ivtot)
...
#compared to odin model
low_eir_odin_baseline_medium <- odin_baseline %>%
  filter(ref == 1 & between(t, medium_t_start, medium_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_baseline = Ivtot)

low_eir_odin_medium <- odin_mda_medium %>%
  filter(ref == 1 & between(t, medium_t_start, medium_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_int = Ivtot)

medium_mda_comp_odin <- left_join(low_eir_odin_baseline_medium, low_eir_odin_medium)
...
baseline_prev_medium <- low_eir_msim_baseline %>%
...
#compared to odin model
low_eir_odin_prev <- odin_baseline %>%
  filter(ref == 1 & t == medium_t_end+30) %>%
  select(t, slide_prev0to5) %>%
  rename(prev_baseline = slide_prev0to5)

low_eir_odin_medium_prev <- odin_mda_medium %>%
  filter(ref ==1 & t == medium_t_end+30) %>%
  select(t, slide_prev0to5) %>%
  rename(prev_int = slide_prev0to5)

medium_mda_comp_odin <- left_join(low_eir_odin_prev, low_eir_odin_medium_prev)
...
#compared to odin model
low_eir_odin_inc <- odin_baseline %>%
  filter(ref == 1 & between(t, medium_t_start, medium_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
  rename(inc_baseline = clin_inc0to5)

low_eir_odin_medium_inc <- odin_mda_medium %>%
  filter(ref ==1 & between(t, medium_t_start, medium_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
  rename(inc_int = clin_inc0to5)

medium_mda_comp_odin <- left_join(low_eir_odin_inc, low_eir_odin_medium_inc)
...
early_baseline_Ivtot <-low_eir_msim_baseline %>%
  filter(between(t, early_t_start, early_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_baseline = Ivtot)
...
#compared to odin model
low_eir_odin_baseline_early <- odin_baseline %>%
  filter(ref == 1 & between(t, early_t_start, early_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_baseline = Ivtot)

low_eir_odin_early <- odin_mda_early %>%
  filter(ref ==1 & between(t, early_t_start, early_t_end)) %>%
  select(t, Ivtot) %>%
  rename(Ivtot_int = Ivtot)

early_mda_comp_odin <- left_join(low_eir_odin_baseline_early, low_eir_odin_early)
...
baseline_prev_early <- low_eir_msim_baseline %>%
...
#compared to odin model
low_eir_odin_prev_early <- odin_baseline %>%
  filter(ref == 1 & t == early_t_end+30) %>%
  select(t, slide_prev0to5) %>%
  rename(prev_baseline = slide_prev0to5)

low_eir_odin_early_prev <- odin_mda_early %>%
  filter(ref ==1 & t == early_t_end+30) %>%
  select(t, slide_prev0to5) %>%
  rename(prev_int = slide_prev0to5)

early_mda_comp_odin <- left_join(low_eir_odin_prev_early, low_eir_odin_early_prev)
...
baseline_inc_early <- low_eir_msim_baseline %>%
  filter(between(t, early_t_start, early_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
  rename(inc_baseline = clin_inc0to5)
...
#compared to odin model
low_eir_odin_inc <- odin_baseline %>%
  filter(ref == 1 & between(t, early_t_start, early_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
  rename(inc_baseline = clin_inc0to5)

low_eir_odin_early_inc <- odin_mda_early %>%
  filter(ref ==1 & between(t, early_t_start, early_t_start+180)) %>%
  select(t, clin_inc0to5) %>%
  rename(inc_int = clin_inc0to5)

early_mda_comp_odin <- left_join(low_eir_odin_inc, low_eir_odin_early_inc)
...
mda_timing_df_long <- mda_timing_df %>%
  pivot_longer(cols = c("msim_mda", "odin_mda"), names_to = "model", values_to = "eff") %>%
  mutate(EIR = "low")

saveRDS(mda_timing_df_long, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/mda_timing_df_long_low_EIR.rds")

#plot comparing efficacy of the different models

mda_timing_df_long <- readRDS("analysis/exploring_interactions/MIM_poster/chapter_plots/mda_timing_df_long_low_EIR.rds")
mda_timing_df_long$model <- factor(mda_timing_df_long$model,
                                   levels = c("odin_mda", "msim_mda"))



efficacy_odin_msim_low_EIR_plot <- ggplot(mda_timing_df_long, aes(x = factor(timing_mda, levels = c("early", "medium", "late")), y  = eff, fill = as.factor(model)))+
  geom_bar(stat = "identity", position = position_dodge(), col = "black")+
  facet_wrap(vars(endpoint),
             labeller = labeller(endpoint = c(
               slide_prev0to5 = "Slide prevalence in under 5-year-olds",
               clin_inc0to5 =  "Clinical incidence in \n under 5-year-olds (per 1000 persons)",
               Ivtot = "Infectious vectors"
             )))+
  theme_bw(base_size = 14)+
  labs(x = "Timing of MDA in relation to ITN campaign", y = "Efficacy (%)")+
  theme(legend.position = c(0.9, 0.8)) +
  scale_x_discrete(labels = c("early" = "6 months", "medium" = "1 year", "late" = "2 years"))+
  ylim(0,100)+
  scale_fill_manual(name = "Model", values = c('#1b9e77','#d95f02'), labels = c("Model B1",
                                                                                "Model C"))
ggsave(efficacy_odin_msim_low_EIR_plot, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/odin_sensitivity_plots_mda_timing_low_eir.pdf")
