...
summary(lm(abs_EIR_add ~ abs_EIR_antag, data = model_compare_EIR)) #R2 is 0.9997

model_comparison_plot_abs_EIR_bb <- ggplot(model_compare_EIR, aes(x = abs_EIR_antag, y = abs_EIR_add, col = as.factor(bites_Bed), group = 1))+
  geom_point(size = 3)+
  scale_colour_manual(values = bb_pal, labels = c("0.25", "0.5", "0.75", "0.9"),
  ...
  annotate("text", label = "italic(R)^2 == 0.999", parse = TRUE, x = 12.5, y = 17.5, col = "red", size = 6)+
  theme(axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12),
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
traits_plot <- ggplot(bb_traits, aes(x = bites_Bed, y = value, col = as.factor(model)))+
...
bb_plots <- cowplot::plot_grid(dynamics_plot_bb, model_comparison_plot_EIR_bb, traits_plot, labels = c("A)", "B)", "C)"), ncol = 3)
...
dynamics_plot_Q0 <- ggplot(antag_mod, aes(x = t/365, y = slide_prev0to5*100, linetype = as.factor(model), col = as.factor(Q0)))+
  ...
  scale_colour_manual(name = "Human blood index", labels = bb_labs, values = Q0_pal)+
  theme(legend.position = c(0.5, 0.8), legend.direction = "vertical")+
  scale_x_continuous(breaks=seq(0, 10, 2))+
  #theme(axis.text.x = element_text(size = 12),
  #      axis.text.y = element_text(size = 12),
...
model_compare_EIR1 <- mods %>%
  select(t, Q0, model, EIR_tot) %>%
  filter(between(t, IVM_start1[1], IVM_start1[3]+23))

model_compare_EIR <- model_compare_EIR1 %>%
  spread(model_compare_EIR1, model, EIR_tot) %>%
  group_by(Q0) %>%
  summarise(mean_EIR_antag_LLIN = mean(antag_LLIN),
            mean_EIR_antag_LLIN_IVM = mean(antag_LLIN_IVM),
            mean_EIR_add_LLIN = mean(add_LLIN),
