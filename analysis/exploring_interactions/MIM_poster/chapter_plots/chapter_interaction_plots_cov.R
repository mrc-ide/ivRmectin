...
#res plots####
antag_mod <- readRDS(file = "analysis/exploring_interactions/MIM_poster/itn_cov/antag.rds")
add_mod <- readRDS(file = "analysis/exploring_interactions/MIM_poster/itn_cov/add.rds")
head(add_mod)
...
#DYNAMICS
cov_labs <- c("0", "20", "60", "80")

dynamics_plot_cov <- ggplot(antag_mod, aes(x = t/365, y = slide_prev0to5*100, linetype = as.factor(model), col = as.factor(itn_cov)))+
  ...
  scale_colour_manual(name = "LLIN usage (%)", values = cov_pal,
                      labels = c("0", "20", "60", "80"))+
...
model_compare_EIR <- mods %>%
  ...
  select(t, itn_cov, model, EIR_tot, itn_cov) %>%
...
model_comparison_plot_abs_EIR_cov <- ggplot(model_compare_EIR, aes(x = abs_EIR_antag, y = abs_EIR_add, col = as.factor(itn_cov), group = 1))+
  geom_point(size = 3)+
  scale_color_manual(values = cov_pal, labels = cov_labs,
                     name = "LLIN usage (%)")+
  ...
  annotate("text", label = "italic(R)^2 == 0.999", parse = TRUE, x = 8, y = 15, col = "red", size = 6)+
  theme(axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12),
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
cov_traits <- antag_mod %>%
  filter(between(t, IVM_start1[1], IVM_start1[3]+23)) %>%
  select(t, avhc, mu, itn_cov) %>%
  group_by(itn_cov) %>%
  summarise(av_time_bloodmeals = 1/mean(avhc), av_life_exp = 1/mean(mu))

cov_traits_antag <- gather(cov_traits, trait, value, av_time_bloodmeals:av_life_exp,
                           factor_key = TRUE) %>%
  mutate(model = "antag")

cov_traits_add <- add_mod %>%
  filter(between(t, IVM_start1[1], IVM_start1[3]+23)) %>%
  select(t, avhc, mu, itn_cov) %>%
  group_by(itn_cov) %>%
  summarise(av_time_bloodmeals = 1/mean(avhc), av_life_exp = 1/mean(mu))

cov_traits_add <- gather(cov_traits_add, trait, value, av_time_bloodmeals:av_life_exp,
                         factor_key = TRUE) %>%
  mutate(model = "add")

cov_traits <- rbind(cov_traits_antag, cov_traits_add)

model_labels <- c(
  `av_time_bloodmeals` = "Average time \n between human bloodmeals",
  `av_life_exp` = "Average mosquito \n life expectancy")

traits_plot <- ggplot(cov_traits, aes(x = as.factor(itn_cov), y = value, col = as.factor(model)))+
  ...
  scale_x_discrete(labels = c("0", "20", "60", "80"))

cov_plots <- cowplot::plot_grid(dynamics_plot_cov, model_comparison_plot_abs_EIR_cov, traits_plot, labels = c("A)", "B)", "C)"), ncol = 3)

ggsave(cov_plots, file = "analysis/exploring_interactions/MIM_poster/chapter_plots/cov_plots.svg")
