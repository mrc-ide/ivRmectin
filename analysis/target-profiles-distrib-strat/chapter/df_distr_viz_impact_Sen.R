...
df_distr_all <- readRDS("analysis/target-profiles-distrib-strat/chapter/output/df_distr_Sen.rds")
...
mv_plot <- ggplot(df_distr, aes(x = (t - start)/365, y = mv, col = as.factor(model_type))) +
  ...
  scale_color_manual(name = "Scenario", values = distr_pals,
                     labels = c("Complete MDA in 1 day",
                                "Baseline",
                                "Complete MDA in 10 days",
                                "Complete MDA in 30 days")) +
  xlim(-0.25, 1) +
  xlab("Years since intervention started") +
  geom_vline(xintercept = 0, col = "black", linetype = "dashed", size = 1.05) +
  geom_vline(xintercept = 0.23, col = "black", linetype = "dashed", size = 1.05) +

  # Arrows added with annotate()
  #all-in-one
  annotate("segment", x = 0, y = 175, xend = 0, yend = 171,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[1], size = 1) +

  #10 day staggered
  annotate("segment", x = times_10_stag[2]/365, y = 175, xend = times_10_stag[2]/365, yend = 171,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[3], size = 1) +

  annotate("segment", x = times_10_stag[3]/365, y = 175, xend = times_10_stag[3]/365, yend = 171,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[3], size = 1) +

  #30d staggered
  annotate("segment", x = times_30_stag[2]/365, y = 175, xend = times_30_stag[2]/365, yend = 171,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[4], size = 1) +

  annotate("segment", x = times_30_stag[3]/365, y = 175, xend = times_30_stag[3]/365, yend = 171,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[4], size = 1)
...
inc_plot <- ggplot(df_distr, aes(x = (t - start)/365, y = clin_inc0to5*1000, col = as.factor(model_type)))+
  ...
  # Arrows added with annotate()
  #all-in-one
  annotate("segment", x = 0, y = 10, xend = 0, yend = 9.6,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[1], size = 1) +

  #10 day staggered
  annotate("segment", x = times_10_stag[2]/365, y = 10, xend = times_10_stag[2]/365, yend = 9.6,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[3], size = 1) +

  annotate("segment", x = times_10_stag[3]/365, y = 10, xend = times_10_stag[3]/365, yend = 9.6,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[3], size = 1) +

  #30d staggered
  annotate("segment", x = times_30_stag[2]/365, y = 10, xend = times_30_stag[2]/365, yend = 9.6,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[4], size = 1) +

  annotate("segment", x = times_30_stag[3]/365, y = 10 , xend = times_30_stag[3]/365, yend = 9.6,
           arrow = arrow(length = unit(0.3, "cm")),
           color = distr_pals[4], size = 1)
...
#measure impact between t start and returns to eqm 0.23y since int started
timy <- 0.23*365
...
impact_overall <- df_distr_wide_setting %>%
  ...
  summarise(tot_cases_baseline = sum(baseline)*1000,
            ...
            tot_cases_all_in = sum(`all-in`)*1000) %>%
...
impact_30_stag <- df_distr_wide_setting %>%
  group_by(init_EIR, Q0) %>%
  filter(between(t, start, start+23 + times_30_stag[3])) %>% #ivm has a killing effect for 23 days
...
impact_10_stag <- df_distr_wide_setting %>%
  group_by(init_EIR, Q0) %>%
  filter(between(t, start, start+23+times_10_stag[3])) %>%
  summarise(tot_cases_baseline = sum(baseline, na.rm = TRUE)*1000,
            tot_cases_10d = sum(`stag-10d`, na.rm = TRUE)*1000,
            tot_cases_30d = sum(`stag-30d`, na.rm = TRUE)*1000,
            tot_cases_all_in = sum(`all-in`, na.rm = TRUE)*1000) %>%
