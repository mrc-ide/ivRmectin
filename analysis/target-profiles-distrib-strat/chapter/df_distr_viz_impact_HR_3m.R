...
unique(df_distr_all$ref)

df_distr_all <- df_distr_all %>%
  mutate(model_type = case_when(model_type == model_types[2]~ "20d-stagger",
                                 TRUE ~ model_type))
...
df_distr <- df_distr_all %>%
  filter(init_EIR == 100 & Q0 == 0.92 & ivm_cov %in% c(covs[2], covs[4]))
...
cumsum(mv_killed_int_stats$killed_10_d_stagger)
cumsum(mv_killed_int_stats$killed_20d_d_stagger)
cumsum(mv_killed_int_stats$killed_all_in_one)
...
plot_perennial <- cowplot::plot_grid(dynamics_perennial, impact_perennial)
